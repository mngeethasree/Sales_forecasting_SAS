...
proc arima data=Work.preProcessedData plots
    (only)=(series(corr crosscorr) residual(corr normal) 
		forecast(forecastonly)) out=work.outARIMA011;
	identify var=sales(1) outcov=work.outcov;
	estimate q=(1) method=ML outest=work.outest outstat=work.outstat 
